...
TEST(FIRFilterKernel, TestSum)
{
  ...
  {
    for (const size_t &nTaps : {4, 8, 16, 32})
    {
        //std::cerr << "Starting: " << fftSize << ", " << nTaps << std::endl;
        CUDA_ERROR_CHECK(cudaDeviceSynchronize());
        std::size_t nSpectra = 1024 * 131072 / fftSize;
        FilterCoefficientsType filterCoefficients(fftSize * nTaps, 1.f);
        thrust::device_vector<float> input((nSpectra + nTaps) * fftSize, 1.f);
...
TEST(Integrate, Integrate)
{
  ...
  for (size_t i =0; (i < nSpectra) && (i < 64); i++)
  {
